...
public partial class CreateSelectorAlternate 
{
    // Replace this line:
    // [RegexGenerator(@"([a-zA-Z_][a-zA-Z0-9_]*)(\[(\d+)\])?", RegexOptions.Compiled)]
    // private static partial Regex GetPropertyRegex();  // Source-generated regex for compile-time optimization

    // With this:
    [System.Text.RegularExpressions.GeneratedRegex(@"([a-zA-Z_][a-zA-Z0-9_]*)(\[(\d+)\])?", RegexOptions.Compiled)]
    
    private static Regex GetPropertyRegex()
    {
        throw new NotImplementedException();
    }
    // Source-generated regex for compile-time optimization

    private static readonly Regex PropertyRegex = GetPropertyRegex();  // Use the generated regex
    ...
    public bool IsRatio => RegressorDicer is RatioVariableDicer || RegressorDicer is LnRatioVariableDicer;
    ...
    private Func<Element, (double x, double y)> BuildSelector()
    {
        var ySelector = BuildYSelector();
        var xSelector = BuildXSelector();
    ...
    private Func<Element, double> BuildYSelector()
    {
        var dependentTarget = DependantDicer.Target;
        return e => Convert.ToDouble(GetNestedPropertyValue(e, dependentTarget) ?? throw new InvalidOperationException($"Property value not found for {dependentTarget}"));
    }

    private Func<Element, (double, double)> BuildXSelector()
    {
        if (IsRatio)
        {
            var numeratorTarget = Numerator?.Target ?? throw new InvalidOperationException("Numerator target is missing");
            var denominatorTarget = Denominator?.Target ?? throw new InvalidOperationException("Denominator target is missing");
            return e => (
                Convert.ToDouble(GetNestedPropertyValue(e, numeratorTarget) ?? throw new InvalidOperationException($"Property value not found for {numeratorTarget}")),
                Convert.ToDouble(GetNestedPropertyValue(e, denominatorTarget) ?? throw new InvalidOperationException($"Property value not found for {denominatorTarget}"))
            );
        }
        else
        {
            ...
            return e => (
                Convert.ToDouble(GetNestedPropertyValue(e, regressorTarget) ?? throw new InvalidOperationException($"Property value not found for {regressorTarget}")),
                0.0  // Placeholder for non-ratio cases
            );
        }
    }
...
// Remove or update this class, as it is not needed and causes confusion with the real attribute:
public partial class RegexGeneratorAttribute
{
...
public class CreateSelector
{
    ...
    private Func<Element, double> CreateYSelector()
    {
        ...
        return e =>
        {
            ...
            return DependantDicer.IsLogarithmic ? Math.Log(val) : val;
        };
    }
    ...
    public Func<Element, (double x, double y)> Selector
    {
        get
        {
            return e =>
            {
                ...
                {
                    x = RegressorDicer.IsLogarithmic ? Math.Log(num) : num;
                }
                return (x, y);
            };
        }
