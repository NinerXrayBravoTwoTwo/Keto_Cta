...
namespace KetoCtaTest
{
    public class SelectorTwoTest(ITestOutputHelper testOutputHelper)
    {
        ...
        public void StaticCompileAttributeNormalizeTest()
        {
            List<Visit> visits =
            [
                new("1", DateTime.Now.AddDays(-365), 100, 50, 20.5, 30.0, 15.0),
                new("2", DateTime.Now, 110, 55, 22.0, 33.0, 16.5)
            ];
            ...
            while (x++ < 3)
            {
                foreach (var attribute in allAttributes)
                {
                    ...
                    var reflectValue =
                        (double)(MazeTwistyPassages.GetNestedPropertyValue(element, compiled.numerator) ?? double.NaN);
        ...
        public void EmptyDenominatorError()
        {
            ...
            // Validate
            testOutputHelper.WriteLine(
                $"Title: {title};   Dep: {selector.DependentCompile} vs Reg: {selector.RegressorCompile}");
        ...
        {
            // Arrange
            var miner = new GoldMiner(
                "TestData/keto-cta-quant-and-semi-quant.csv"); // Ensure this file exists and is valid
